...
function generateClass(def, classTypesByName){
	...
	clazz.prototype._makeLike = function(json, w, makeCb, equalsFunc){
		...
		for(var j=0;j<propPrototypes.length;++j){
			...
			var existingValue = this[pp.propertyName]
			
			//console.log('making like: ' + pp.propertyName)
			...
			if(v && pp.isObject){
				...
				if(!v._id || pp.def.type.inner){
					...
					if(!equalsFunc || !existingValue || !equalsFunc(existingValue, n)){
						//console.log(!!existingValue + ' ' + equalsFunc(existingValue, n))
						this[pp.propertyName] = n
					}else{
						//console.log('making like: ' + n.yamlId)
						existingValue._makeLike(v, w, makeCb, equalsFunc)
					}
				}else{
					//console.log('here')
					if(equalsFunc && existingValue && equalsFunc(existingValue, v)){
						existingValue._makeLike(v, w, makeCb, equalsFunc)
						v = existingValue
					}//else{
				...
				if(v){
					...
					for(var i=0;i<v.length;++i){
						...
						if(!cv._id || pp.def.type.members.inner){
							...
							if(existingValue){
								for(var k=0;k<existingValue.length;++k){
									var other = existingValue[k]
									if(equalsFunc(other, cv)){
										substituted[k] = true
										//console.log('making like in: ' + n.yamlId)
										other._makeLike(v[i], w, makeCb, equalsFunc)
										cv = other
										break;
									}
								}
					...
					if(existingValue){
						//console.log('got existing value: ' + existingValue.length)
						for(var k=0;k<existingValue.length;++k){
							if(substituted[k]) continue
							//console.log('including existing: ' + JSON.stringify(existingValue[k].toJson()))
							n.push(existingValue[k])
						}
					}//else{
					//	console.log('no existing value: ' + pp.propertyName);
					//}
					this[pp.propertyName] = n
				}
			...
			if(v && pp.isObject){
				...
				if(v){
					for(var i=0;i<v.length;++i){
						...
						if(!cv._id || pp.def.type.members.inner){
							if(!cv.type) throw new Error('no type given: ' + JSON.stringify(cv))
							cv = makeCb(cv.type, cv)
						}
						n.push(cv)
					}
